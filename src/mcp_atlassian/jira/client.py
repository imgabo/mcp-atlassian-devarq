...
class JiraClient:
    ...
    def delete_comment(self, issue_key: str, comment_id: str) -> bool:
        ...
        Returns:
            True if the comment was deleted successfully, False otherwise
        """
        try:
            # La API de atlassian-python-api no expone delete_comment directamente,
            # así que usamos la llamada REST.
            url = f"/rest/api/2/issue/{issue_key}/comment/{comment_id}"
            response = self.jira.delete(path=url)
            # La API retorna 204 No Content en éxito
            return response is None or response == ''
        except Exception as e:
            logger.error(f"Error deleting comment {comment_id} from {issue_key}: {str(e)}")
            return False
